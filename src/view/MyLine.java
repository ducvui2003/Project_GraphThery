...
//Vẽ cạnh liên thông nối giữa 2 đỉnh
	public void drawLine(Graphics2D g, Point p1, Point p2, Color colorCost,
		...
		g.setStroke(new BasicStroke(size));//set độ dày cạnh
		double theta = Math.atan2(p2.y - p1.y, p2.x - p1.x);
		g.draw(l);//Vẽ cạnh
//		Mặc định sẽ là vẽ cạnh cho đơn đồ thị
		if (type && cost >= 0) {
		...
		g.setColor(colorCost);//Chỉ set được màu cho content, ko set cho graphíc
//		Sử dụng để vẽ chữ thể hiện chi phí đường đi
		g.drawString(c, (int) (Math.abs(p1.x + p2.x) / 2),
				(int) (p1.y + p2.y) / 2);
	}
//	Xác định xem điểm đó có chứa trong đa giác đã tạo hay ko ?
	public boolean containerPoint(Point p) {
		Polygon poly = createPolygon(l);//Class đa giác
...
//	Tạo một hình đa giác để chứa các đỉnh
	private Polygon createPolygon(Line2D line) {
		...
		Polygon poly = new Polygon(x, y, 4);//Tạo đa giác 4 cạnh
		return poly;
	}
